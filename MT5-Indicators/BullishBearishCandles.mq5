   ...
   for(int i = MathMax(prev_calculated-1, 0); i<rates_total; i++)
     {
      ...
      total_range = MathMax(high[i] - low[i], 0.00000000000001);
      up = MathMax(open[i], close[i]) - (2*total_range/3 + low[i]);
      up = MathMax(0, MathMin(1, up));
      down = low[i] + total_range/3 - MathMin(open[i], close[i]);
      down = MathMax(0, MathMin(1, down));
      Power[i] = (up - down)/(get_indicator_value_by_handle(atr_handle)/3);
